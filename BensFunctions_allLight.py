...
def getOrbitRange(files):
    ...
    for i in range(len(files)):  #Loop over all the files, len(files) gives the number of files
        orbit = files[i][:-36][-5:]  #this indexes the orbit value, as a string, out of each filename
...
def getSpecifics(files, setShape, allOrbits, orbitRange, Type):
    ...
    exps = np.array(exps)
    captureTimes = np.array(captureTimes)
    #sza = np.array(sza)

    if len(Orbits) == 0:
        print('No orbits of shape {} in orbit range {}-{} of type {}'.format(setShape, orbitRange[0], orbitRange[1], Type))
    
    return (Bins, Files, Shapes, Orbits, headers, captureTimes, exps) #return the bins, files, shapes, and orbits
...
def makeCube(files, setShape, spatLim):
    """
    ...
    FourDCube = firstprim_data#[:, :, spatLim:]
...
def getHist(cube, threshold=6, limit=False):
    """
    ...
    vmi = cube.min() #Calculate the minimum and maximum of the cube
    print(vmi)
    vma = cube.max()
...
def plotPostage(cube, orbits, setShape, captureTimes, plotRange):
    """
    ...
    for i in range(len(orbits)): #loop over all orbits
        time = captureTimes[i][j][2][-17:-3]
        times.append(str(time, 'utf-8'))
        plt.tight_layout()
        data = cube[j, :, i, :]
        if orbits[i] <= plotRange[1]:
            if orbits[i] >= plotRange[0]:
                plt.subplot(1, 8, stop) #make a new subplot

                plt.title("orbit {}, {} \n {}".format(orbits[i], j, time.decode('utf-8'))) #Set title
...
def plotPostageHist(cube, orbits, setShape, cubeMode, std, plotRange, captureTimes, rmsThres, orbEvent, binwidth=100, threshold=6):
    """
    ...
    plt.figure(figsize=(20, 3), dpi=100) #Make a figure
    plt.tight_layout(w_pad=6) #Make each subplot compacts
    plt.tight_layout()    
    for i in range(len(orbits)): #loop over all orbits
        plt.tight_layout()
        if orbits[i] >= plotRange[0]:
            if orbits[i] <= plotRange[1]:
                ...
                if len(setShape) == 3: #Determione how to index cube based on the shape of the inputted cube
                    flatData = cube[j, :, i, :].flatten() #Flatten the data to make a histogram
                    typicalData = cube[j, :, -1, :].flatten() #Make a typical file too
                ...
                plt.subplot(1, 5, stop)
                plt.tight_layout()
                b = orbits[i] #get the orbit

                #if i == 0 and j == 0: #set title and labels
                plt.xlabel("log DN")
                ...
                vals1, bins1, patches1 = plt.hist(flatData, histtype="step", bins=range(int(vmi),int(vma+binwidth),int(binwidth)), label='E', color='blue')
                plt.tight_layout()
                #make a "typical" histogram for comparison
                vals, bins, patches = plt.hist(typicalData, histtype="step", bins=range(int(vmi),int(vma+binwidth),int(binwidth)), label='T', color='orange')
                plt.tight_layout()
                plt.axvline(x=(cubeMode+(std*threshold)), color="k", label="""{}\u03C3""".format(threshold)) #make a line at the 6 sigma threshold
                rms = round(np.sqrt(np.average(vals1**2)), 1)
                ...
                #if i == j == 0: #only plot the legend on the very first image
                plt.legend(loc='best', prop={'size': 6})
                plt.title("orbit {}, {}, \n STD: {}, RMS: {} \n {}".format(b, j, imgSTD, rms, time))
                plt.xlabel('log DN')
...
def findOutliers(orbits, cube, cubeMode, std, setShape, threshold=6):
    """
    ...
    for i in range(len(orbits)):#Loop over all the orbits
        ...
        countOrbs.append(orbits[i])
    
    counts = np.array(counts) #make the lists arrays for easier computation later
    percents = np.array(percents)
    medians = np.array(medians)
    countOrbs = np.array(countOrbs)
    
    return (counts, percents, medians, countOrbs)




def findPlotSTDs(orbits, cube, setShape, spatLim):#, vmi, vma):
    """
    This function finds the number of "outliers" in an image, i.e. the number of pixels that are above a threshold
    I.e. the number of bins that are above 6 sigma of a typical FUVdark image, by making a "mask"
    A mask deletes values less than a specific values
    ...
    for i in range(len(orbits)):#Loop over all the orbits
        #for j in range(setShape[0]):
        img = cube[j, :, i,  spatLim[0]:spatLim[1]]
        
        vals, bins, patches = plt.hist(img.flatten(), histtype="step", range=(vmi,vma), bins=int(vma-vmi))
...
def plotOutliers(orbits, counts, percents, medians, THRESHOLD=6):
    """
    ...
    plt.title('{}\u03C3 Outliers vs Orbit Number (first dark only)'.format(THRESHOLD))
    ...
    plt.xlabel('Orbit number')
    plt.title('Log {}\u03C3 Outliers vs Orbit Number (first dark only)'.format(THRESHOLD))
    plt.show()
    ...
    plt.xlabel('Orbit number')
    plt.title('Percent {}\u03C3 Outliers vs Orbit Number (first dark only)'.format(THRESHOLD))
    plt.tight_layout()
    ...
    plt.xlabel('Orbit number')
    plt.title('Median vs Orbit Number (first dark only)'.format(THRESHOLD))
    plt.tight_layout()
    plt.show()
...
def plotPrettyMedians(orbits, medians, dates, c, m, s, saveName, Type, step, frameOff=False):
    """
    This makes a custom plot of orbits vs percent counts (#Outliers/total pixel count) for comparison to other 
    figure (i.e. from papers, etc)
    The figure has 2 x-axes: the lower is orbit # and the upper is the corresponding date, the y axis is on the right side
    ...
    ax.yaxis.tick_left() #set the y-axis positions
    ...
    plt.scatter(orbits, medians, label=Type+' Light', marker=m, c=c, s=s) #scatter plot of orbits vs percents
    ...
    plt.legend(loc='upper left', frameon=False); #show legend
    plt.savefig(str(saveName), bbox_inches='tight', transparent=True) #save the figure and show the plot
...
def plotTemps(orbits, files):
    ...
    plt.title('Case & Detector Temperature vs Orbit #')
    plt.show()
    
    
    
    
def runAll(directory, Type, shape, orbitRange, spatLim, plotRange, rmsThres, orbEvent, c, m, s, step, saveName, dates, fT):
    """
    This executes all functions, in sequence
    """
    
    files = getFiles(Type, directory)
    ...
    bins, files, shapes, orbits, headers, captureTimes, exps = getSpecifics(files, shape, orbits, orbitRange, Type)

    print('\n \nNumber of orbits with shape {} and type {}: {}'.format(shape, Type, len(orbits)))
    print('Number of fileswith shape {} and type {}: {}'.format(shape, Type, len(files)))
    
    if len(orbits) != 0:
        Cube = makeCube(files, shape, spatLim) #Make a cube
        Min, Max, cubeStd, cubeMode = getHist(Cube) #get the min and max of the cube, std, and mode

        times = plotPostage(Cube, orbits, shape, captureTimes, plotRange) #Plot the images
        IJpop = plotPostageHist(Cube, orbits, shape, cubeMode, cubeStd, plotRange, captureTimes, rmsThres, orbEvent) #plot histograms
        
        counts, percents, medians, countOrbs = findOutliers(orbits, Cube, cubeMode, cubeStd, shape)
        
        plotOutliers(countOrbs, counts, percents, medians)
                    
        allSTDs, orbStds = findPlotSTDs(orbits, Cube, shape, spatLim)#, vmi, vma)
        
        plotPrettyMedians(countOrbs, medians, dates, c, m, s, saveName, Type, step, fT)
        plotTemps(orbits, files)

        return (Cube, files, headers, counts, percents, medians, countOrbs, times, cubeMode, cubeStd, captureTimes, allSTDs, orbStds, IJpop, exps)
