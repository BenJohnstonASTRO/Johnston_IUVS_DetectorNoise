...
def getSpecifics(files, setShape, allOrbits, orbitRange, Type):
    ...
    return (Bins, Files, Shapes, Orbits, headers, exposures) #return the bins, files, shapes, and orbits
...
def plotPostage(cube, orbits, setShape, dark, headers):
    """
    ...
    for i in range(len(orbits)): #loop over all orbits
        for j in range(setShape[0]):
            ...
            plt.title("{}, {}".format(orbits[i], j)) #Set title
...
def runAll(directory, Type, dark, shape, orbitRange, scale, dates, name0, name1, loc, lim0, lim1, c, m, s, step, F=False, pAll=True):
    """
    ...
    bins, files, shapes, orbits, headers, exposures = getSpecifics(files, shape, orbits, orbitRange,  Type)
    ...
    if len(orbits) != 0:
        ...
        cubeCounts, cubePercents, cubeMedians, countOrbs = findOutliers(orbits, Cube, cubeMode, cubeStd, dark, shape) #get the counts
        if pAll == True:
            plotPostage(Cube, orbits, shape, dark, headers) #Plot the images

            plotOutliers(countOrbs, cubeCounts, cubePercents, cubeMedians, dark) #plot the orbits vs outliers
        
            plotTemps(orbits, files) #make orbit vs case temp and orbit vs detector temp plots

        return (Cube, cubeCounts, cubePercents, cubeMedians, countOrbs, orbits, files, headers, exposures, cubeStd, cubeMode)
