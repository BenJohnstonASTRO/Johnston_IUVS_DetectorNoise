...
def plotPostage(cube, orbits, setShape, dark):
    ...
    for i in range(len(orbits)): #loop over all orbits
        ...
        plt.imshow(np.log10(data), aspect=setShape[-1]/setShape[-2], origin="lower", cmap="CMRmap", vmin=np.log10(vmi), vmax=np.log10(vma))
        if i !=0:
            plt.axis('off')
        if i == len(orbits)-1:
            cbar = plt.colorbar(fraction=0.056)
...
def runAll(directory, Type, dark, shape, orbitRange, scale, dates, name0, name1, loc, lim0, lim1, c, m, s, step, frame, \
           ALL=True, postage=True, F=False):
    """
    ...
    if len(orbits) != 0:
        ...
        if postage == True:
            ...
            plotPostageHist(Cube, orbits, shape, dark, cubeMode, cubeStd) #plot the histograms
        ...
        if ALL == True:
            #"pretty" plots of ouliers vs log(counts) and outliers vs percent counts
            plotPrettyLogCounts(orbits, cubeCounts, dates, c, m, s, str(name0), loc, lim0, Type, step, dark, frameOff=frame)
            plotPrettyPercents(orbits, cubePercents, dates, c, m, s, str(name1), loc, lim1, Type, step, dark,  frameOff=frame)
